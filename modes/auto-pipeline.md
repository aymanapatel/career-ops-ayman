...
Execute exactly like `offer` mode (read `modes/oferta.md` for all blocks A-F + Block G Posting Legitimacy).
...
Register in `data/applications.md` with all columns including Report and PDF in ✅.

**If any step fails**, continue with the remaining ones and mark the failed step as pending in the tracker.
