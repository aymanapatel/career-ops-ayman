...
13. Determine `{company}` (kebab-case slug of company name, e.g. "JPMorgan" → "jpmorgan")
14. Extract `{job-id}` from the JD if present:
...
15. Create directory `output/{company}/{job-id}/` (use `node lib/get-output-dir.mjs "{company}" [job-id]` to get the path and create it)
16. Write HTML to `output/{company}/{job-id}/cv.html`
17. Run: `node generate-pdf.mjs output/{company}/{job-id}/cv.html output/{company}/{job-id}/cv.pdf --format={letter|a4}`
...
**NEVER add skills the candidate doesn't have. Only reword real experience using the exact vocabulary of the JD.**
...
| `{{SUMMARY_TEXT}}` | Custom summary with keywords |
...
b. **IMMEDIATELY** download the PDF using Bash:
   ```bash
    curl -sL -o "output/{company}/{job-id}/cv-canva.pdf" "{download_url}"
   ```
   The export URL is a pre-signed S3 link that expires in ~2 hours. Download it right away.
c. Verify the download:
   ```bash
    file output/{company}/{job-id}/cv-canva.pdf
   ```
