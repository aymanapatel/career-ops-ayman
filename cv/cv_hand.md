...
Senior Full-stack engineer with 6 years of experience at Mastercard building production systems in Java and React. Led the architecture of building microfrontend for customer onboarding platform and serve as a lead engineer for many cross-functional projects. Recently completed MSc in Advanced Computer Science at Queen Mary University of London. 
...
- **[Nebius Project](https://github.com/aymanapatel/nebius-project)** -- Built a Repository Summarizer service using FastAPI and LLMs that generates human-readable summaries of GitHub repositories. Uses Abstract Syntax Trees (ASTs) with implementation using Tree-sitter to extract context of code using code definitions with a specified token limit. This project secured admission into the Nebius AI Engineering Fellowship
- **[Go Microservices](https://github.com/aymanapatel/golang-microservices)** -- Built a end-to-end stack for Go microservices with OpenAPI documentation, Gorilla Mux with custom request validation, Envoy-based Layer 4/ Layer 7 load balanacing, and gRPC communication between microservice.
- **[Java CLI using GraalVM](https://github.com/aymanapatel/java-cli)** -- Built a Java CLI tool using the GraalVM and Picocli, then compiled it to a native binary with GraalVM with sub-zero startup time and a cross-platform native binary
